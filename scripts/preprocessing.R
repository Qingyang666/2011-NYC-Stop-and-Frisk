...
# IMPORT RAW DATA
sqf.data <- read_csv('data/2011.csv')

# 0) drop columns that won't be used, either because they're irrelevant or seem to be coded badly
sqf.data <- sqf.data %>% select(-recstat, -officrid, -sumoffen, -compyear, 
                                -comppct, -dettypcm, -linecm, -forceuse)

# 1) date and time: combine datestop and timestop into timestamp
...
sqf.data <- sqf.data %>% mutate(inside = recode.io(inout), 
  ...
  select(-inout, -perobs, -detailcm, -offverb, -offshld, -arstoffn)
...
sqf.data <- sqf.data %>% mutate(location.housing = 
                                  recode.factor(sqf.data$trhsloc, c('P', 'H', 'T'), 
...
# 6) recode y/n variables: physical force used by officer
...
# 7) recode y/n variables: reason for stop
...
# 8) recode y/n variables: reason for frisk
...
# 9) recode y/n variables: additional circumstances
...
# 10) recode y/n variables: basis of search
...
# 11) recode y/n variables: results of frisk/search
...
# combine ht_feet and ht_inch into height and rename weight
...
# add extra columns into data set for weapon found or gun found
...
sqf.data <- sqf.data %>% mutate(day = wday(timestamp, label = T, abbr = F),
                                ...
                                time.period = case_when(hour(timestamp) < 4 ~ '1',
...
# check proportion of NAs in each column by year
proportion.nas <- sqf.data %>% 
  group_by(year) %>% 
  summarize_all(funs(nas = sum(is.na(.))/n()))

# write out missingness summary table
write_csv(proportion.nas,'data/missingness.csv')

# write out data set after preprocessing
write_csv(sqf.data,'data/sqf_2011.csv')
